...
import pricingPlansData from '@/data/mock/pricing.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetPricingPlans') || query.includes('nodePricingPlans')) {
      return loadMockData('pricing.json')
    }
