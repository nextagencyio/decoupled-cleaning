...
interface PricingPlanCardProps {
  item: DrupalPricingPlan
}

export default function PricingPlanCard({ item }: PricingPlanCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).billingPeriod && (
            <p className="text-sm text-primary-700 font-medium mb-2">{(item as any).billingPeriod}</p>
          )}
